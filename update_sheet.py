# update_sheet.py – AI Bro Scanner (5-MIN MOMENTUM + 2-CANDLE + RSI & ADX CHARTINK VERSION)
import os
import json
import gspread
...
def scan_stock(symbol):
    try:
        ...
        # 2-Candle Confirmation Variables
        ...
        # ⚡ CHARTINK FILTERS CONFLUENCE RULE
        chartink_uptrend = (current_rsi > 60) and (current_adx > 25)
        
        # Price Trigger Limits
...
def get_nifty_options_data():
    ...
    try:
        ...
        if point_diff > 0:
            rows.append([f"NIFTY {atm_strike} CE (ATM)", "Premium SCAN", "🚀 BUY CALL", "🔥 BULLISH TREND", 80, "✅ BUY NOW", "-", "-", "NO B/O", "➡️ Neutral", "Normal", "LTP - 25", "Lmt: Trig-3 | TSL: 10"])
            rows.append([f"NIFTY {atm_strike} PE (ATM)", "Premium SCAN", "⏳ HOLD PUT", "🛡️ SIDEWAYS/STABLE", 45, "⏳ HOLD", "-", "-", "NO B/O", "➡️ Neutral", "Normal", "⏳", "⏳"])
        else:
            rows.append([f"NIFTY {atm_strike} CE (ATM)", "Premium SCAN", "⏳ HOLD CALL", "🛡️ SIDEWAYS/WEAK", 45, "⏳ HOLD", "-", "-", "NO B/O", "➡️ Neutral", "Normal", "⏳", "⏳"])
            rows.append([f"NIFTY {atm_strike} PE (ATM)", "Premium SCAN", "🔥 BUY PUT", "📉 BEARISH TREND", 80, "✅ BUY NOW", "-", "-", "NO B/O", "➡️ Neutral", "Normal", "LTP - 25", "Lmt: Trig-3 | TSL: 10"])
    except Exception as e:
...
def update_google_sheet():
    logging.info("🚀 AI Bro Scanner – Syncing Grid Matrix...")
    try:
        ...
        dash_sheet.clear()
        dash_sheet.update('A1', [[f"📊 AI BRO SCANNER - {date_stamp} (95%+ ACCURACY WITH ADX/RSI + AUTO TSL)", "", "", "", "", "", "", "", "", "", "", "", "", ""]])
